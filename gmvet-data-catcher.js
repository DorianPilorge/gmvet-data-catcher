...
// Attend que les infos soient disponibles
function waitInfos() {
  return new Promise(function (resolve, reject) {
    (function waitForInfos(){
        if (document.getElementsByName('species.id')[0].value &&
            document.getElementsByName('gender.id')[0].value) return resolve();
        setTimeout(waitForInfos, 30);
    })();
  });
}
...
function addWarning() {
  ...
  loader.setAttribute('style', 'display: block; margin: 8px; margin-left: 105px; color: #505050; font-weight: bold;');
  loader.innerHTML = "Afin d\'exporter les informations, il est nécessaire de raffraichir la page (F5)";
  container.appendChild(loader);
}
...
function addError() {
  ...
  loader.innerHTML = "Impossible d'exporter les informations";
  container.appendChild(loader);
}
...
// Injecte le script d'exportation
...
if (window.location.href.indexOf('medicalpatient') > -1) {
  try {
    ...
    waitInfos().then(function() {
      getInfos();           // On récupère les informations
      formatInfos();        // On les formattent avant de les afficher
      formatEmpty();        // On vérifie les champs vides
      loader.remove();
      addButton();
      addInvoice();
      addPrescription();
      injectScript();       // On inject le script sur la page
